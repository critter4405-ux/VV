...
> Stand: Bau-Verifikation 25.09.2026, Branch `feat/c1-kontext-signatur`, frische PostgreSQL 16.13. **Gate gesperrt** bis Vier-Augen-Review (GPT-6 Sol + Gemini 3.1 Pro) konvergiert und der Betreiber per Pull Request freigibt.
...
| 3 | **Fälschung und Ablauf** verweigert (Signatur, Kennung, `exp`, Nutzlast) | c1-asserts „DoD3“ (14 Varianten + Ablauf mitten in der Transaktion) | erfüllt |
| 4 | **Mandantentrennung:** Ticket A öffnet nie B | c1-asserts „DoD4“ · [c1-e2e.txt](c1-e2e.txt) (Mandant B im Token → 403) | erfüllt |
| 5 | **Einmal-Tickets:** zweite verbindliche Aktion verweigert, Lesen 60 s mehrfach | c1-asserts „DoD5“ (seriell, gleiche Transaktion, parallel, Freigabe, 8 Befehle statisch) | erfüllt |
...
| 10 | **Validatoren + Selbsttest** grün; neue Proben „keine Direkt-Grants an vv_app“, „keine `set_config('app.…')` in der App“ | [validator-live.txt](validator-live.txt) · [validator-report.json](validator-report.json) · [selftest.txt](selftest.txt) (22/22, davon 6 C-1) | erfüllt |
...
| 12 | **Typecheck/Tests** Web/Worker/Ticket grün, `npm audit` 0, `docker compose config` valide, **CI auf GitHub grün** | [typecheck.txt](typecheck.txt) · [test-ticket.txt](test-ticket.txt) · [npm-audit.txt](npm-audit.txt) · [compose-config.txt](compose-config.txt) · CI: im Pull Request | lokal erfüllt · CI im PR |

**Zusatz:** Migrationen 0006–0013 idempotent ([idempotenz.txt](idempotenz.txt), danach C-1-Proben erneut 70/70: [c1-asserts-nach-idempotenz.txt](c1-asserts-nach-idempotenz.txt)) · RLS-Leistung gemessen ([perf.md](perf.md)).
