...
// noch über vv_audit_log(): Actor = geprüfter Ticket-Akteur (C-1, via withTenant), Serverzeit, reservierte Fach-Präfixe gesperrt — erlaubt sind nur `app.*` / `policy.*`.
...
export async function writeAudit(
  ...
  // Muss INNERHALB von withTenant(client, ticket, fn) laufen (geprüfter Kontext gesetzt).
