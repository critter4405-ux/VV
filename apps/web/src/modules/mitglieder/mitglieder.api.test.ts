...
// Der Principal wird hier im TEST-Server gesetzt (simuliert ein verifiziertes OIDC-Token); der
// Produktionscode kennt keinen solchen Header — dort kommt er nur aus verifyBearer().
...
async function setup() {
  ...
  server = createServer(async (req, res) => {
    ...
    await handleM05(req, res, { tenantId: TENANT, actor });
  });
...
test("M05-API end-to-end (echte DB)", { skip: !HAS_DB && !process.env.VV_REQUIRE_DB ? "kein DATABASE_URL (lokal)" : false }, async () => {
  assert.ok(HAS_DB, "VV_REQUIRE_DB gesetzt, aber kein DATABASE_URL");
  await setup();
  // Synthetische Person (BASIS-01, vv_app darf Personen anlegen)
  ...
  assert.equal(r.status, 403, "ohne Actor -> deny");
