...
- 26.09.2026 — **C-1 Kontext-Signatur** ([VV-SEC-01](VV-SEC-01.md), PR #21 → `master` `73455a3`): Audit-Akteur (`vv_audit_log`), Antragsteller (`requested_by`) und Freigeber (`vv_decide_approval`) stammen jetzt aus dem **geprüften Ticket-Akteur** statt aus einer frei setzbaren GUC; Freigabe/Ablehnung verbraucht ein Einmal-Ticket; Worker handelt nur als `system:worker`; jeder Schlüsselwechsel wird in die Audit-Kette jedes Mandanten geschrieben (ohne Geheimnis). Restrisiko C-1 geschlossen.
