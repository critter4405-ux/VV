# Bau-Dossier — VV-ORG · Organisationsregister (juristische Personen)
...
- **Code:** VV-ORG
- **Name:** Organisationsregister (juristische Personen)
...
*(zu füllen beim Bau)* Ziel dieses Bausteins: Firmen, Sponsoren, Institutionen getrennt vom Personenregister führen.

## 3. Warum so

*(zu füllen beim Bau)* Hintergrund/Alternativen → ADR-Verweis: [ADR-01](../adr/ADR-01.md)

## 4. Wie umgesetzt

*(zu füllen beim Bau)* Architektur, Datenmodell, Schnittstellen, Grenzen. Referenz: [ADR-01](../adr/ADR-01.md)
...
```mermaid
flowchart LR
    P["Problem"] --> L["VV-ORG<br/>Lösung"]
    L --> N["Nutzen"]
```

## 8. Nutzen in Klartext

Sponsoring/Rechnungen/Kooperationen ohne die Consent-Logik natürlicher Personen zu stören.
