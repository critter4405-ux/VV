...
13. **Reparaturrunde 2 (Review R2, Register P54):** Sind die Fixes vollständig und nicht umgehbar? Attestation nur bei bekannten, verschiedenen Modellfamilien (H-1) · Outbox-Claim nur für Topics aus dem DB-Register `outbox_consumer`, auch bei direkter SQL-Nutzung als `vv_worker` (H-2) · Legal Hold mit Outbox-Ereignis (M-1) · Tagesjob parallelfest (M-2) · Freigabe-FKs mandantendicht (N-1). **C-1** (Vertrauensgrenze `vv_app`) ist eine dokumentierte Design-Grenze (Betreiber-Entscheidung, Stage-1 vor S3) — bitte nur melden, wenn ein Weg über den **normalen HTTP-Pfad** (verifiziertes Token) besteht.
